...
<p class="hero-subtitle">Versión limitada: solo FERTILIDAD ACTUAL y ANÁLISIS DE TEXTURA</p>
</div>
</div>
""", unsafe_allow_html=True)
...
if st.session_state.email_autorizado is None:
    ...
    st.write("Ingresa tu email para probar la versión limitada (2 análisis).")
    email = st.text_input("Email", placeholder="tu@email.com")
    if st.button("✅ Usar la demo"):
        if "@" in email and "." in email:
            st.session_state.email_autorizado = email
            st.success(f"¡Bienvenido! Puedes usar la demo.")
            st.rerun()
        else:
...
with st.sidebar:
    ...
    # === SOLO DOS ANÁLISIS ===
    analisis_tipo = st.selectbox("Tipo de Análisis (DEMO):", ["FERTILIDAD ACTUAL", "ANÁLISIS DE TEXTURA"])
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# A PARTIR DE AQUÍ, EL CÓDIGO ES EXACTAMENTE IGUAL AL ORIGINAL (FUNCIONES Y LÓGICA PRINCIPAL)
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
